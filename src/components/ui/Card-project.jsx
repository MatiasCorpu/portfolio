    ...
    <Card3d classNameAditional={"ml-2 text-sm font-mono text-slate-700 dark:text-zinc-200"} clickAnimation={"false"}>
      ...
      <div>
        <div className="relative w-full">
          <div className={`absolute flex justify-between items-center w-full z-5 font-semibold py-2 px-3 ${backgroundTitle === "dark" ? "text-zinc-300" : "text-slate-700"}`}>
            <h1>{title}</h1>
        ...
        >
          <div className="flex w-full justify-between">
            <h2 className="font-semibold items-center">Descripción</h2>
            <IconChevronDown className={`text-slate-600 dark:text-zinc-500 active:rotate-180 transition-all ease-in-out duration-200  ${isExpanded ? "rotate-180": ""}`} />
          </div>

          <div className={`overflow-hidden ease-in-out duration-300 ${isExpanded ? "max-h-40" : "max-h-0"}`}>
            <div className="pt-3 font-thin text-sm text-slate-700 dark:text-zinc-400">
